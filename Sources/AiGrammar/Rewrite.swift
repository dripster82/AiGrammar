...
final class RewriteController {
    ...
    init(monitor: FocusMonitor, models: ModelManager, prompts: PromptStore,
    ...
    func rewriteSelection(preset: RewriteInstruction? = nil) {
        ...
        popover.show(original: target, range: nsRange, element: element,
                     ...
                     makeStream: { [weak self] instruction in
                         ...
                         return self.engine().rewrite(target, instruction: instruction, systemPrompt: systemPrompt)
                     })
    }
    ...
    private func selectionBounds(element: AXUIElement, range: CFRange) -> CGRect? {
        if let ax = AX.boundsForRange(element, location: range.location, length: max(range.length, 1)) {
            let composer = AX.frame(element)
            let sane = ax.width > 0 && ax.height > 0 &&
                (composer.map { $0.insetBy(dx: -8, dy: -60).contains(CGPoint(x: ax.midX, y: ax.midY)) } ?? true)
    ...
    static func resolve(_ choice: String, models: ModelManager) -> RewriteEngineChoice {
        func usableLocal(_ id: String) -> ModelInfo? {
            guard LlamaServer.isInstalled,
                  ...
                  models.path(forID: id) != nil else { return nil }
            return m
        }
        switch choice {
        case "cleanup":
...
final class RewriteSession: ObservableObject {
    ...
    func run(_ instruction: RewriteInstruction) {
        ...
        output = ""
        streaming = true
        sawThinking = false; thinkingSeconds = 0; totalSeconds = 0
        start = Date()
    ...
    private func consume(_ partial: String) {
        if partial == "Thinking…" {
            if phase != .thinking { phase = .thinking; sawThinking = true }
    ...
    private func finish() {
        ...
        Log.write("[rewrite] generation complete (total \(formatDuration(totalSeconds))\(sawThinking ? ", thinking \(formatDuration(thinkingSeconds))" : ""))")
    }

    func cancel() { task?.cancel(); streaming = false; ticker?.invalidate(); ticker = nil }
...
final class RewritePopoverController {
    ...
    func show(original: String, range: NSRange, element: AXUIElement, engineName: String,
        ...
        let view = RewriteView(
            ...
            accept: { [weak self] text in
                self?.onAccept?(range, element, text); self?.hide(reason: "accepted")
            },
            close: { [weak self] in self?.hide(reason: (self?.session?.streaming ?? false) ? "cancelled" : "rejected") })
    ...
    func showMessage(_ message: String) {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 3) { [weak self] in self?.hide(reason: "message timeout") }
    }
    ...
    private func present(_ hosting: NSView, at rect: CGRect?, width: CGFloat) {
        ...
        outsideClickMonitor = NSEvent.addGlobalMonitorForEvents(matching: [.leftMouseDown, .rightMouseDown]) { [weak self] _ in
            ...
            if !panel.frame.contains(NSEvent.mouseLocation) {
                ...
                if !self.isPinned { self.hide(reason: "clicked outside, generation done + engaged") }
            }
        }
        if let rect {
...
struct RewriteView: View {
    ...
    private var content: some View {
        VStack(alignment: .leading, spacing: 10) {
            HStack {
                Label(isCleanup ? "Clean up" : "Rewrite",
                      systemImage: isCleanup ? "textformat.abc" : "sparkles").font(.headline)
                Spacer()
    ...
    private var cleanupPicker: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            Button { session.run(.fixGrammar) } label: {
                Label("Clean up text", systemImage: "textformat.abc")
                    .frame(maxWidth: .infinity)
            }
            .buttonStyle(.borderedProminent)
            Text("For real rewriting, pick a model in AI Models (Apple on-device or a local model).")
    ...
    private var presetPicker: some View {
        VStack(alignment: .leading, spacing: 6) {
            Text("Choose how to rewrite the selection:")
                .font(.caption).foregroundStyle(.secondary)
            ForEach(Array(RewriteInstruction.presets.enumerated()), id: \.offset) { index, instruction in
                Button { session.run(instruction) } label: {
                    HStack {
        ...
        ScrollView {
            if session.output.isEmpty || session.output == "Thinking…" {
                HStack(spacing: 6) {
                    ...
                    Text(session.output == "Thinking…" ? "Thinking…"
